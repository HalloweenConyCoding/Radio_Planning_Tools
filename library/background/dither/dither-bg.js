(function attachConyDither(global) {
  "use strict";

  const DEFAULTS = {
    waveColor: [0.8274509803921568, 0.7843137254901961, 1.0],
  ...
  const fragmentShaderSource = `
    ...
    vec3 dither(vec2 fragCoord, vec3 color) {
      ...
      float stepSize = 1.0 / max(u_colorNum - 1.0, 1.0);
  ...
  function resolvePointerTarget(target, fallback) {
    if (!target) return fallback;
    if (target === "window") return global;
  ...
  function createShader(gl, type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      throw new Error(message || "Unable to compile dither shader.");
    }
  ...
  function createProgram(gl, vertexSource, fragmentSource) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      ...
      throw new Error(message || "Unable to link dither shader program.");
    }
  ...
  function create(target, options) {
    ...
    if (!container) {
      throw new Error("ConyDither target was not found.");
    }
    ...
    canvas.style.cursor = settings.enableMouseInteraction ? "crosshair" : "default";
    ...
    const uniforms = {
      ...
      enableMouseInteraction: gl.getUniformLocation(program, "u_enableMouseInteraction"),
    ...
    let pointerSource = resolvePointerTarget(settings.pointerTarget, canvas);
    const state = {
      destroyed: false,
      paused: false,
      rafId: 0,
    ...
    function updateMouse(event) {
      ...
      mouse.x = (event.clientX - rect.left) * dprX;
      mouse.y = (event.clientY - rect.top) * dprY;
      mouse.active = true;
    }
    ...
    function render(now) {
      ...
      const elapsed = settings.disableAnimation ? 0 : (now - state.startTime) / 1000;
      const mouseX = mouse.active ? mouse.x : canvas.width * 0.5;
      const mouseY = mouse.active ? mouse.y : canvas.height * 0.5;
      ...
      gl.uniform3f(uniforms.waveColor, settings.waveColor[0], settings.waveColor[1], settings.waveColor[2]);
      gl.uniform2f(uniforms.mouse, mouseX, mouseY);
      gl.uniform1i(uniforms.enableMouseInteraction, settings.enableMouseInteraction ? 1 : 0);
      gl.uniform1f(uniforms.mouseRadius, settings.mouseRadius);
      gl.uniform1f(uniforms.colorNum, settings.colorNum);
      gl.uniform1f(uniforms.pixelSize, Math.max(settings.pixelSize, 1.0));
    ...
    function setOptions(nextOptions) {
      const previousPointerSource = pointerSource;
      Object.assign(settings, nextOptions || {});
      canvas.style.cursor = settings.enableMouseInteraction ? "crosshair" : "default";
      pointerSource = resolvePointerTarget(settings.pointerTarget, canvas);
      if (pointerSource !== previousPointerSource) {
        previousPointerSource.removeEventListener("pointermove", updateMouse);
        if (previousPointerSource === canvas) {
          previousPointerSource.removeEventListener("pointerleave", onPointerLeave);
        }
        pointerSource.addEventListener("pointermove", updateMouse, { passive: true });
        if (pointerSource === canvas) {
    ...
    function destroy() {
      ...
      pointerSource.removeEventListener("pointermove", updateMouse);
      if (pointerSource === canvas) {
